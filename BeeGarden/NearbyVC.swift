...
struct PhoneNumber {
    ...
    static func extractAll(from string: String) -> [PhoneNumber] {
        PhoneDataDetector.find(all: .phoneNumber, in: string)
            .compactMap {  PhoneNumber(string: $0) }
    }

    static func first(in string: String) -> PhoneNumber? {
        guard let phoneNumberString = PhoneDataDetector.first(type: .phoneNumber, in: string) else { return nil }
